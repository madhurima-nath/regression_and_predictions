...
Regularization fits a model containing all $p$ predictors using a technique that constrains or regularizes the coefficient estimates, or shrinks the coefficient estimates to zero.
...
[**Ridge Regularization**]{.ul} coefficients are values that minimize
...
-   $\lambda \rightarrow 0$ : least square model with all $p$ predictors

-   $\lambda \rightarrow \infty$ : null model with all no predictors
...
-   as $\lambda$ increases, $L^2 norm$ of $\beta$ coefficients will always decrease
