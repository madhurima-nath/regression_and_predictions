...
These are categorical variables. Most important advantage of converting categorical life to factor variable: they can be used in statistical modeling where they will be implemented correctly, i.e., storing data as factors ensures that the modeling functions will treat such data correctly, and then be assigned the correct number of degrees of freedom.
...
-   In both methods, the following idea is used:
    ...
    Delete the observations one at a time, refit the regression model on the remaining 𝑛–1n--1 observations each time. Then, compare the results using all 𝑛n observations to the results with the 𝑖i-th observation deleted to see how much influence the observation has on the analysis. This enables to assess the potential impact each data point has on the regression analysis.
